	...
	if( Gallery_2J_Duplicate_Only( $post->post_type ) ){
		$actions['clone'] 				= 
			'<a href="'.TwoJGalleryFieldsDuplicate_getUrl( $post->ID , 'display', false)	
				.'" title="'.esc_attr(TwoJGalleryHelper::_tr("Clone this item", 'blog-mechanics-theme-gallery')). '">'
					.TwoJGalleryHelper::_tr('Clone', 'blog-mechanics-theme-gallery')
			.'</a>';
		$actions['edit_as_new_draft'] 	= '<a href="'.TwoJGalleryFieldsDuplicate_getUrl( $post->ID )						.'" title="'.esc_attr(TwoJGalleryHelper::_tr('Copy to a new draft', 'blog-mechanics-theme-gallery')).'">'	.TwoJGalleryHelper::_tr('New Draft', 'blog-mechanics-theme-gallery')	.'</a>';
	}
	return $actions;
}
...
function TwoJGalleryFieldsDuplicateDo($post, $status = '', $parent_id = '') {
	...
	if ($title == '') $title = TwoJGalleryHelper::_tr('Untitled');
		...
		if( Gallery_2J_Duplicate_Only($post->post_type)) { ?>
			<div id="twoj-gallery-duplicate-action">
				<a class="submit_twoj_gallery_duplicate " href="<?php echo TwoJGalleryFieldsDuplicate_getUrl( $_GET['post'] ); ?>">
					<?php TwoJGalleryHelper::_et('Copy to a new draft', 'blog-mechanics-theme-gallery'); ?>
				</a>
			</div>
		<?php
		}
